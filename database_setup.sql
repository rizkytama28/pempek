-- 1. Buat tabel untuk kategori menu
...
-- 3. Masukkan beberapa data kategori contoh
INSERT INTO categories (name, slug)
VALUES
  ('Pempek', 'pempek'),
  ('Kuah', 'kuah');

-- 4. Masukkan beberapa data menu contoh
-- Supabase akan otomatis mencocokkan category_id
...
-- 5. Aktifkan Row Level Security (PENTING UNTUK KEAMANAN)
ALTER TABLE menus ENABLE ROW LEVEL SECURITY;
ALTER TABLE categories ENABLE ROW LEVEL SECURITY;

-- 6. Buat "policy" agar semua orang bisa MEMBACA data menu dan kategori
CREATE POLICY "Public can read categories" ON categories FOR SELECT USING (true);
CREATE POLICY "Public can read menus" ON menus FOR SELECT USING (true);

-- Nanti kita akan buat policy khusus untuk admin agar bisa menulis/mengubah data
